...
Esta dualidade é a vantagem competitiva. O concorrente de referência (**Dossier do Treinador**) é **apenas individual** (uma equipa por conta, sem partilha editável entre contas). O Mister é individual **e** plataforma de clube.
...
Sessões completas pré-construídas (aquecimento + parte principal + jogo reduzido + retorno à calma), com durações e objetivos. Curadas pela equipa Mister (seed) e criadas pelo treinador/clube. Organizadas por objetivo tático, fase da época, escalão/idade e parte do treino.
...
model Planeamento {
  ...
  nome       String?         // nome livre da semana formalizada ("Semana do torneio", "Pré-jogo Benfica"); opcional
  modoSemana ModoSemana?     // ESTRUTURADO (dias MD-X) | TEXTO_LIVRE; só quando o treinador formaliza a semana (tipo=SEMANAL). Nullable = não formalizada
...
model Sessao {
  ...
  planeamentoId String?     // Só válido quando tipoSessao == NORMAL (imposto no servidor: superRefine + guarda nas actions)
  data          DateTime
  duracaoMin    Int?
  objetivo      String?
  local         String?
  ...
  momentoSemana MomentoSemana? // posição do dia na semana no modo ESTRUTURADO (MD-X); opcional, nunca bloqueia a sessão
...
model SessaoExercicio {
  ...
  parteTreino ParteTreino?
  notas       String?
  // Snapshot só-de-leitura (mecanismo em 4.2.1 — IMPLEMENTADO 2026-08-16). Preenchido AUTOMATICAMENTE quando se
  // adiciona à sessão um exercício com proprietario = TREINADOR: congela os dados relevantes para que o CLUBE
  // reconstrua o plano histórico mesmo depois de o treinador (e o master editável) saírem. Imutável — não se edita nem apaga.
  snapNome      String?
...
Geração server-side de **imagens quadradas (1080×1080 PNG)** prontas a publicar no Instagram, fechando o loop de conteúdo orgânico (UGC) sem o treinador ter de fazer *print* e cortar. Três tipos de card, todos com o **emblema/cor do clube** e a **marca Mister discreta** no rodapé:

- **`resultado`** — placar do jogo (`Clube vs Adversário`, resultado grande, data · escalão · competição).
- **`mvp`** — jogador em destaque do jogo (nome + jogadas relevantes: golos, assistências e defesas se for guarda-redes). O MVP é eleito por pontuação sobre as `EstatisticaAtleta` do jogo (golo=3, assistência=2, defesa=0,5; desempate por golos e depois assistências).
- **`ranking`** — Top 5 marcadores do escalão/época (agregação por `atletaId`, evitando fundir homónimos — §10.2).

**Implementação (sem novo schema Prisma):**

- **Rota de imagem** `GET /api/social/card` (runtime Node — Prisma; `dynamic = "force-dynamic"`), usa `ImageResponse` de **`next/og`** para desenhar o card. Parâmetros: `tipo` (`resultado`|`mvp`|`ranking`), `jogoId` (resultado/mvp) ou `escalaoId`+`epocaId` (ranking), e `token`.
- **Agregação** em `lib/social/card-data.ts` (`server-only`): lê o mínimo necessário via Prisma, **sempre com scoping ao clube autenticado** (`obterClubeIdAtual`) e verificação de leitura do escalão (`podeLerEscalao`). Devolve `RespostaCard<T>` com `status` HTTP.
- **Token anti-scraping** (`lib/social/token.ts`): HMAC-SHA256 determinístico do recurso (`gerarTokenCard`/`validarTokenCard`/`urlCard`), gerado no servidor e embutido no URL de partilha. **Não** usa segredos de autenticação (segredo dedicado `SOCIAL_CARD_SECRET`, com degradação graciosa em dev). Não substitui a sessão — é uma segunda barreira contra enumeração.
- **Botões de partilha:** `components/social/BotoesPartilhaJogo.tsx` («Partilhar resultado» — só com resultado registado — e «Ver MVP») em `/jogos/[id]`; `components/social/BotaoPartilhaRanking.tsx` («Partilhar ranking») em `/escaloes/[id]/analiticos`. Abrem a imagem numa nova aba (`<a target="_blank">`); o treinador guarda-a a partir do browser (sem download automático).
- **Schema partilhado** `lib/schemas/social.ts`: `cardQuerySchema` (Zod) + `eEscalaoFormacaoJovem`.

**RGPD (regra dura, §3.14):** os cards **nunca** expõem dados de atletas menores. Escalões de **formação jovem** — sub-14 e inferiores (deteção tolerante a grafias: "Sub-12", "sub 12", "SUB12") e categorias tradicionais (petizes, traquinas, benjamins, escolas, minis) — são **bloqueados**: a rota devolve **`403`** com «Cards sociais não disponíveis para escalões de formação jovem (RGPD)» e os botões **não são sequer renderizados** para esses escalões (dupla proteção: servidor + UI).
...
- **Treinador Principal** — `PROPRIOS_ESCALOES`, todas as capacidades de dados de equipa dos seus escalões + `EXERCICIOS_GERIR` + `RELATORIOS_VER` + `COMUNICACOES_GERIR` (para comunicar a convocatória via WhatsApp — caso de uso nº1 da semana). `PROMOVER_ATLETAS` desligada por defeito (concedível por override).
...
- **Biblioteca de exemplo curada** (Mister): organizada por parte do treino (aquecimento/principal/retorno), objetivo tático e escalão/idade — garante que **nunca começa vazia**. O botão de instalação vive no **estado vazio da aba Clube**.
...
- **Export CSV (F1):** os analíticos de **escalão** e de **atleta** são **exportáveis em CSV** para Excel/folha de cálculo. **Serialização das estruturas já calculadas** (`AnaliticoEscalao`/`AnaliticoAtleta`) — zero recálculo, mesmas permissões (`RELATORIOS_VER` + leitura do escalão). Formato pensado para o **Excel PT-PT**: BOM UTF-8 (acentos), separador `;`, escape RFC 4180, números com ponto decimal. Helper puro `lib/utils/csv.ts` (`paraCsv`/`juntarBlocosCsv`) + Server Actions `exportarAnaliticoEscalaoCsv`/`exportarAnaliticoAtletaCsv` (`lib/actions/analise.ts`), devolvendo `{ csv, nomeFicheiro }`. **Escalão:** tabela por atleta (golos, assistências, jogos utilizados, minutos acumulados + 1 coluna por métrica configurável) + resumo do escalão (V/E/D, golos, sessões, taxa de presença média — métrica de equipa, não por atleta). **Atleta:** evolução jogo a jogo + linha de totais + resumo da época (minutos acumulados, taxa de presença, métricas agregadas). **Botão de download na UI (F1.3):** `components/analiticos/ExportarCsvBotao.tsx` (cliente) — recebe a Server Action já parametrizada (via `.bind`), chama-a, cria um `Blob('text/csv;charset=utf-8;')` (o BOM já vem no CSV) e dispara o download com o `nomeFicheiro`; toast de erro (Sonner), estado de loading, ação secundária discreta (`variant="outline"`, `size="sm"` ≥44px, `print:hidden`). Colocado no cabeçalho dos analíticos do escalão (`app/(app)/escaloes/[id]/analiticos/page.tsx`, ao lado de «Gerar relatório partilhável») e do atleta (`app/(app)/plantel/[id]/page.tsx`, aba Analíticos, rótulo «Exportar histórico», só quando há escalão de contexto). **PDF por impressão (F1.4):** reforço do bloco `@media print` em `app/globals.css` — oculta `nav`/`aside` e blocos marcados com `[data-print-hidden]` (o `<header>` **não** é ocultado por seletor genérico, para a identidade do clube do relatório partilhado sair na impressão), expande `main` a toda a largura, evita quebra de página dentro de `table`/`tr` e garante `[data-print-logo]` visível; a via de «Guardar como PDF» é o `window.print()` já existente (`components/relatorios/BotaoImprimir.tsx`) na vista partilhável `/r/[token]`.
...
**Agenda agregada e deteção de conflitos de pavilhão (F3):**
- **Agenda agregada (P2.2):** vista única, cronológica e filtrável por escalão dos treinos e jogos de **todos os escalões** do clube (`lib/actions/agenda.ts::obterAgendaClube`) — para o Diretor Técnico/Admin acompanhar a atividade sem navegar escalão a escalão. Leitura agregada de `Sessao` + `Jogo`, sem entidade nova.
- **Aviso de conflito de pavilhão (F3):** como `local` é texto livre, dois escalões podem marcar treino/jogo no mesmo pavilhão à mesma hora sem se aperceberem. A app deteta a sobreposição e **avisa, sem bloquear**:
  - **Helper puro** (`lib/utils/agenda-conflitos.ts`): normaliza o `local` (trim + minúsculas + colapso de espaços; "Pavilhão A" ≡ "  pavilhao   A ") e deteta sobreposição da janela `[data, data + duração)` (intervalo semiaberto — eventos adjacentes não colidem). Duração assumida `DURACAO_PADRAO_MIN = 90` min quando não há `duracaoMin` (caso dos jogos). `local` vazio → nunca conflito.
  - **Pré-verificação** (`lib/actions/agenda.ts::verificarConflitoAgenda`): atravessa **todos os escalões do clube**, exclui o próprio evento em edição (`excluirId`) e devolve os conflitos sem nunca impedir a escrita.
  - **Aviso na UI (F3.3):** `SessaoForm` (`tipo: "TREINO"`) e `JogoForm` (`tipo: "JOGO"`) chamam a pré-verificação **após-debounce (~800ms)** quando `data` + `local` (+ `duração`, nos treinos) estão preenchidos. Havendo sobreposição, mostra-se um **banner âmbar não-bloqueante** ("Possível conflito de pavilhão") com o tipo de evento, o escalão e a hora de cada conflito. É **distinto de um erro de validação** (âmbar, não vermelho) e **permite guardar na mesma**. Sem `local`, ou em falha de rede, não há aviso (silêncio total — funcionalidade secundária).
...
- **Implementado (P2.4):** página `/perfil` (`app/(app)/perfil/page.tsx`, Server Component) com identidade do treinador (nome + email) e o **histórico de carreira** editável. Acesso pelo menu do utilizador (`BarraTopo` → "Perfil e carreira").
  - **Modelo `RegistoCarreira`** (`prisma/schema.prisma`): passagem de carreira do treinador com `clube` (texto livre — pode ter treinado fora do sistema), `escalao`, `epocaInicio`, `epocaFim` (null = em curso), `conquistas`, `notas` e `ordem` (ordenação manual). **Pertence ao `Utilizador` (a pessoa), não ao clube** — é **portátil** e acompanha o treinador entre clubes (âncora do §17.3, "o que crias é teu para toda a carreira"). `onDelete: Cascade` no utilizador.
  - **Server Actions** (`lib/actions/perfis.ts`): `obterRegistosCarreira` (lista do utilizador autenticado, `ordem` desc), `criarRegistoCarreira`, `atualizarRegistoCarreira` e `eliminarRegistoCarreira`. Todas verificam `auth()` primeiro e as de escrita fazem **ownership check** (`RegistoCarreira.utilizadorId === session.user.id`) antes de escrever; revalidam `/perfil`. Validação com Zod em `lib/schemas/perfil.ts`. Como o histórico é da pessoa (portátil), estas actions **não** filtram por clube nem por época.
  - **UI** (`components/perfil/`): `ListaRegistosCarreira` (Server Component; lista + estado vazio), `RegistoCarreiraForm` (cliente; criar/editar), `RegistoCarreiraAcoes` (cliente; editar em diálogo + eliminar com confirmação) e `NovoRegistoCarreira` (cliente; botão + diálogo de criação).
- **Implementado (P4.5):** enriquecimento do perfil sobre a base de P2.4 — **sem alterações ao schema** (zero colunas/migrações novas).
  - **Métricas de carreira agregadas** (`obterResumoCarreira` em `lib/actions/perfis.ts`, tipo `ResumoCarreira`): resumo do percurso calculado em JS sobre o `findMany` (sem SQL de agregação), filtrado pelo `utilizadorId` do utilizador autenticado. Devolve `totalRegistos`, `clubesDistintos` (nomes únicos, case-insensitive), `epocasAtivas` (registos sem `epocaFim`), `conquistasTotal` (registos com `conquistas` não vazias) e `primeiraEpoca` (`epocaInicio` mais antiga, ou null).
  - **Painel de métricas** em `/perfil` (`app/(app)/perfil/page.tsx`): três cards `grid-cols-3` antes da lista — "X épocas em Y clubes", "Z conquistas" e "Desde AAAA/AAAA" (primeira época). Só aparecem quando há registos. Tokens suaves da marca (`bg-cinza-50` / `bg-laranja-50`), número grande + label pequena.
  - **Copiar link do perfil** (`components/perfil/CopiarLinkPerfil.tsx`, cliente): botão no cabeçalho que copia `${origin}/perfil/{userId}` para o clipboard via `navigator.clipboard.writeText()` + toast "Link copiado!". **Não** cria coluna `partilhaPublica` nem rota nova — a partilha pública propriamente dita continua FUTURO (requer mais design).
  - **FUTURO (P4.x):** ingestão automática do percurso a partir dos dados do sistema, conquistas agregadas e partilha pública com página dedicada.
...
> **Decisão 2026-08-12 (P4.8):** monitorização de carga percebida e prevenção de lesões baseada em evidência (sRPE + rácio agudo:crónico), sem módulo clínico.

- **RPE da sessão (`Sessao.rpeSessao`, 1-10):** o treinador atribui à sessão um valor de esforço percebido na **escala CR10** (1 = muito leve … 10 = máximo). Registado na página do treino (após presenças) por quem tem `TREINOS_GERIR` no escalão. É opcional — sessões sem RPE não contam para a carga.
- **RPE individual (`RpeAtleta`, 1-10):** cada atleta pode reportar o seu próprio esforço percebido para uma sessão (upsert; um valor por atleta/sessão). Alimenta o **ACWR por atleta** (F2): a carga percebida individual é `RpeAtleta.rpe × Sessao.duracaoMin`, e cada série corre pela mesma função pura `calcularCargaSemanal`.
- **Carga da sessão (sRPE):** `duracaoMin × rpeSessao`. **Carga semanal:** soma das cargas das sessões da semana (semana ISO, começa à segunda-feira).
- **ACWR (Acute:Chronic Workload Ratio):** `carga da semana atual ÷ média da carga das até 4 semanas anteriores`. Indicador de risco de lesão por variação abrupta de carga:
  - **`< 0.8` → subcarga** (destreino / recuperação excessiva);
  - **`0.8–1.3` → zona ideal** (progressão segura);
  - **`> 1.3` → risco de sobrecarga** (pico agudo face ao histórico).
  - Sem histórico suficiente (nenhuma semana anterior com carga) o ACWR fica indefinido.
...
```
Inclui métricas **desativadas** com valores históricos (nunca se apagam — §9). `[]` quando não há valores registados. **UI (P1.9):** `PainelAtleta` mostra uma secção **«Métricas personalizadas»** (nome · total · média/jogo por métrica); quando `metricas` está vazio apresenta o estado vazio «Nenhuma métrica personalizada registada.».
...
```
**Rankings de métricas configuráveis (`obterAnaliticoEscalao` → `rankingsMetricas`)** — para cada `MetricaConfig` com valores na equipa, agrega por atleta (NUMERO → Σ; BOOLEANO → nº registos ≠ 0; ESCALA → média) e devolve `rankingsMetricas: Array<{ metrica, tipo, top: Array<{ atletaId, atletaNome, valor }> }>` — top 10 por valor decrescente, ordenado por `MetricaConfig.ordem`; omite métricas sem atletas com valor > 0. `[]` quando não há valores. **UI (P1.9):** `PainelEscalao` mostra uma secção **«Ranking por métrica»** (novo `RankingsMetricas.tsx`, client) — seletor `shadcn/ui` para escolher a métrica e lista ordenada dos melhores atletas; só é renderizada quando existem rankings (`rankingsMetricas.length > 0`).

**Filtro por competição (P2.5)** — `obterAnaliticoEscalao(escalaoId, epocaId?, competicaoId?)` aceita um `competicaoId` opcional para separar contextos (campeonato / taça / particulares). Quando indicado, filtra **os jogos e tudo o que deles deriva** (resultados V/E/D, golos, marcadores/assistentes, mais utilizados, eventos por tipo e rankings de métricas) por `{ competicaoId }`; **treinos e presenças ficam globais** (são transversais à competição). Sem `competicaoId` mistura todos os contextos (comportamento por defeito, «Todas»). `obterCompeticoesEscalao(escalaoId, epocaId?)` devolve as competições **com jogos** do escalão/época (`{ id, nome, tipo }[]`, ordenadas por tipo e nome) para alimentar o seletor. Ambas exigem `RELATORIOS_VER` + leitura do escalão.
...
Prescritivo. Base Tailwind + shadcn/ui. **Marca do produto: Mister** (guia em `docs/BRAND.md`). Princípio: **a marca é fixa; a cor do clube é dinâmica**.

### 12.0 Design Direction (decisão 2026-08-05)
> A direção visual define o "carácter" do produto. É prescritiva — não reinterpretar.

**Base visual — tema escuro:** o **tema escuro é a base do produto** (é o *default*). Fundo principal **`#0F0E13`** (preto quente, não puro); superfícies em **`#1C1B22`** (cartões) e **`#2A2933`** (superfícies elevadas/hover). Laranja Mister **`#F0531E`** como acento primário. Tipografia **Bricolage Grotesque com presença** — números de estatísticas em tamanhos **grandes e bold** (o dado é protagonista). **Alternância claro/escuro (decisão 2026-08-06, F14):** existe um **alternador** (lua/sol na barra de topo) que permite ao utilizador escolher o tema claro; a preferência é persistida (`localStorage`, via `next-themes`, atributo `class`). O **escuro continua a ser o default** e o carácter do produto; o claro é uma cortesia de acessibilidade/preferência. *(Ajusta a nota anterior "sem alternância claro/escuro".)*

**Cor do clube como identidade:** no contexto de clube, a **sidebar** e os **acentos** adotam as **duas cores dominantes do clube** (`--cor-primaria` e `--cor-secundaria`); o **logótipo do clube** está presente. A interface "pertence" ao clube. Na versão **Individual sem clube (clube técnico)**, o **laranja Mister domina**.
...
- Logótipo do produto (barra de topo/login) — só Mister. Logótipo do clube **presente** (sidebar + marca de água centrada `.club-watermark`) + nome no cabeçalho de identidade.
...
- **Escrita concorrente (last-write-wins):** As server actions **não** implementam optimistic locking. Em cenário de edição simultânea do mesmo registo por dois utilizadores, a última escrita prevalece **sem aviso** (nem deteção de conflito, nem campo `criadoPor`/`atualizadoPor` na maioria dos modelos). Este comportamento é **aceite para o MVP** dado o perfil de utilização (essencialmente um treinador por escalão, com eventual adjunto; edição simultânea do mesmo registo é rara e as operações críticas — grelha de estatísticas de jogo — têm latência baixa, minimizando a janela de conflito). Decisão a **revisitar** quando o produto tiver dados de usage que confirmem o problema (ex.: conflitos reportados por utilizadores). **Futuro (não implementar agora):** campo `version` + verificação de `updatedAt` antes de escrever, aplicado aos modelos de alta escrita (`EstatisticaAtleta`, `Sessao`, `Jogo`), e opcionalmente `criadoPor`/`atualizadoPor` nesses modelos para trilho de auditoria.
...
`criarAtletasEmMassa`; fluxo guiado (plantel em massa → sessão de template → primeira convocatória partilhada). Valor nos primeiros 10 minutos. **P1.6 ✅** `criarClube()` semeia época ativa + escalão-semente «Seniores» (não deixa o clube sem contexto). **P1.7 ✅** wizard `/onboarding` acionado após criar clube (`CriarClubeForm` → `router.push("/onboarding")`).
...
- **2026-08-18** — **Contacto na landing: formulário substituído por email direto (`app/page.tsx`).** A secção `#contacto` da landing deixa de ter formulário e passa a apresentar um **email de contacto direto** com link `mailto:`. **UI (`app/page.tsx`):** a secção (última antes do footer, `scroll-mt-20`, link «Contacto» no cabeçalho mantém-se) passa a **fundo escuro** (`#0F0E13` + gradiente radial laranja, coerente com o hero/footer), com ícone `Mail`, título **«Fala connosco»**, texto curto («Tens dúvidas, sugestões ou queres saber mais sobre o Mister? Envia-nos um email.») e um **botão de destaque** (laranja de ação `#C7430F`, Bricolage Grotesque via `font-display`, alvo de toque ≥44px) com `href="mailto:goncalo.pereira.1992@gmail.com?subject=Contacto%20Mister"` e rótulo `goncalo.pereira.1992@gmail.com`. **Remoções:** apagados o Client Component `components/landing/FormularioContacto.tsx` (e a pasta `components/landing/`, agora vazia), a Server Action `lib/actions/contacto.ts` e o schema `lib/schemas/contacto.ts` (Zod); removida a dependência `resend` do `package.json` (e as env vars `RESEND_API_KEY`/`RESEND_FROM` do `.env.example`) — deixa de haver envio de email server-side, pelo que não são precisos serviço nem chaves de terceiros. **Sem alteração ao schema Prisma, base de dados, permissões ou auth.** Verificação: `npm run typecheck` (0 erros), `npm run build` (`next build` exit 0; a etapa `prisma migrate deploy` não corre no sandbox por a BD Supabase estar inacessível, como nas entradas anteriores), `npm run test` (todos passam), `npm audit --omit=dev` (0 vulnerabilidades).
- **2026-08-17** — **Implementação da UI «Semana de trabalho» (§8.9.1).** Materialização em código da camada de apresentação, sobre os schemas/actions das entradas abaixo. **Terminologia:** «Microciclo»/«Mesociclo» deixam de aparecer na UI — a lista de treinos e a periodização passam a falar de **«Semana»**. **Lista de treinos (`app/(app)/treinos/page.tsx`):** a vista de lista agrupa as sessões **por semana ISO** (segunda–domingo), da mais recente para a mais antiga, reutilizando os helpers puros de `lib/semana.ts` (`segundaFeira`/`domingo`/`numeroSemana`/`semanaSobrepoePlaneamento`) e cruzando com `listarPlaneamentos` para herdar o nome da semana (fallback «Semana N» calculado desde o início da época, coerente com `obterSessoesPorSemana`); cada cabeçalho de semana mostra número + intervalo de datas + nome (quando formalizada) e um botão **«+ Treino»** que pré-preenche a data; a semana atual aparece mesmo quando vazia, com estado convidativo; funciona com a barra de tabs de escalão (incl. «Todos») e preserva local/nº de exercícios/presenças e o *badge* MD-X por sessão; a vista de calendário mantém-se. **Formulário de sessão (`components/treinos/SessaoForm.tsx`):** removido o *dropdown* «Planeamento» e o aviso âmbar (a associação à semana é agora automática pela data, no servidor); campos reordenados (Data → Local → Momento na semana → Objetivo/Notas), com «Tipo de sessão» despromovido para campo secundário; novo Select **«Momento na semana»** (MD-X, opcional, só treino NORMAL) que persiste em `Sessao.momentoSemana`; o cabeçalho de semana pré-preenche a data via `?data=`. **Formulário de planeamento (`components/treinos/PlaneamentoLista.tsx`):** novo campo **«Nome da semana» (opcional)** no topo; *toggle* **`modoSemana`** «Estruturado (dias MD-X)»/«Texto livre» (só SEMANAL) — em Texto livre mostra o campo `notaSemana`, em Estruturado explica que o MD-X se marca por sessão; «Microciclo» renomeado para **«Semana (nº)»**; **Mesociclo** escondido por defeito num bloco **«Avançado»**; a lista de planeamentos passa a mostrar o nome/«Semana N» e deixa de expor Meso/Micro. **Não toca em auth.** Verificação: `npm run typecheck` (0 erros), `npm run lint` (0 avisos), `npm run test` (1037/1037), `npm run build` (`next build` exit 0 com heap alargado; a etapa `prisma migrate deploy` não corre no sandbox por a BD Supabase estar inacessível, como nas entradas anteriores).
- **2026-08-17** — **Implementação da camada de Server Actions «Semana de trabalho» (§8.9.1).** Materialização em código das actions que consomem os campos de modelo adicionados na entrada abaixo. **Schemas Zod:** em `lib/schemas/planeamento.ts` — `nome` (≤100, opcional), `modoSemana` (`enum ESTRUTURADO|TEXTO_LIVRE`, *default* `TEXTO_LIVRE`), `notaSemana` (≤500, opcional) + `MODOS_SEMANA`/`LABEL_MODO_SEMANA`; em `lib/schemas/treino.ts` — `momentoSemana` (`enum` opcional) no `sessaoSchema` + `MOMENTOS_SEMANA`/`MomentoSemana`/`LABEL_MOMENTO_SEMANA` (partilhado com o schema Prisma). **Helpers puros (`lib/semana.ts`, novo):** `segundaFeira`, `domingo`, `numeroSemana` (fórmula `Math.ceil((difDias+1)/7)` ancorada na segunda da época — todas as sessões da mesma semana ISO partilham número) e `semanaSobrepoePlaneamento`. **Periodização (`lib/actions/periodizacao.ts`):** `criarPlaneamento`/`atualizarPlaneamento` passam a persistir `nome`/`modoSemana`/`notaSemana`; `sugerirPlaneamento` devolve `nome` (`"Semana N"` calculado desde o início da época). **Agregação (`lib/actions/semana.ts`, novo):** `obterSessoesPorSemana({ escalaoId })` agrupa as sessões da época ativa por semana ISO (segunda–domingo), cruza com os planeamentos sobrepostos para herdar nome/modo (fallback «Semana N»), expõe o momento característico quando unânime, e ordena da semana mais recente para a mais antiga (`SemanaComSessoes[]`/`SessaoResumo[]`). **Sessões (`lib/actions/treinos.ts`):** `criarSessao`/`atualizarSessao` persistem `momentoSemana` e fazem **auto-associação silenciosa** ao planeamento (só sessões NORMAL sem `planeamentoId` explícito, cuja data cai no intervalo de um planeamento do escalão/época — senão fica `null`; a sessão nunca é bloqueada pela ausência de semana). **Testes:** `tests/semana.test.ts` (12 — helpers puros, agrupamento/ordenação/herança de `obterSessoesPorSemana` e auto-associação de `criarSessao`). **Não toca em auth.** Verificação: `npm run typecheck` (0 erros), `npm run test` (1037/1037).
- **2026-08-17** — **Implementação do schema «Semana de trabalho» (§3.5, §8.9.1).** Materialização em código dos campos de modelo especificados na decisão de produto (A) de 2026-08-16 (que os deixara ⚠️ «a validar tecnicamente»). **Schema (`prisma/schema.prisma`):** adicionados ao modelo `Planeamento` os campos `nome String?`, `modoSemana ModoSemana?` e `notaSemana String?` (`@db.Text`); ao modelo `Sessao` o campo `momentoSemana MomentoSemana?`; e os enums `ModoSemana { ESTRUTURADO TEXTO_LIVRE }` e `MomentoSemana { MD_MENOS_3 MD_MENOS_2 MD_MENOS_1 MD_MAIS_1 ATIVACAO TAPER LIVRE }`. Todos **opcionais/nullable** — `modoSemana` fica **nullable sem default** (conforme §3.5: null = semana não formalizada; só preenchido quando o treinador formaliza), removidos os marcadores ⚠️ em §3.5. **Migração:** `prisma/migrations/20260817120000_add_semana_trabalho/migration.sql` (CREATE TYPE dos 2 enums + ALTER TABLE ADD COLUMN IF NOT EXISTS dos 4 campos; **aditiva, sem perda de dados**). **Não toca em auth.** Verificação: `npx prisma validate` (schema válido), `npx prisma generate` (client regenerado), `npm run typecheck` (0 erros), `npm run test` (1025/1025). Nota: a etapa de aplicação à BD (`prisma migrate deploy`) não corre no sandbox por a BD Supabase estar inacessível, como nas entradas anteriores.
- **2026-08-16** — **Implementação da UI do wizard «Nova Época» (§8.21) — cenários A/B/C.** Materialização em código da camada de apresentação de §8.21 (nenhuma alteração ao texto de §8.21 nem à camada de servidor, já implementada na entrada abaixo). **Componente (`components/definicoes/WizardNovaEpoca.tsx`, novo — Client Component):** wizard multi-passo com indicador de progresso («Passo X de Y» + barra de pontos) e rodapé Anterior/Próximo/ação-final; a validação por passo reutiliza os schemas Zod do servidor (`novaEpocaStep1Schema`, `novoClubeSchema`) — fonte única cliente/servidor. Escolhe o fluxo pelo `cenario` devolvido por `verificarElegibilidadeWizard`: **cenário C** (licença INDIVIDUAL) → fluxo «novo clube» de 3 passos (dados do clube com *color picker*; toggles do que transportar — exercícios/modelos táticos/métricas, todos «sim» por defeito; confirmação) chamando `criarNovoClube`; **cenários A/B** (mesmo clube) → fluxo *rollover* de 3-4 passos (Época: nome + datas + escalões que continuam via checkboxes + toggle «há promoções?»; Plantel: lista de atletas com checkbox «transita?» a ✅ por defeito, número editável, «Marcar/Desmarcar todos» quando >15; Promoções — passo opcional só quando o toggle está ligado — blocos origem→destino com sugestão por idade via `sugerirPromocoes` e confirmação individual, atletas promovidos ficam bloqueados na lista regular para respeitar a sobreposição do servidor; Resumo com contagem «transitam / ficam fora / promovidos») chamando `criarEpocaRollover`. Após sucesso: `toast` + `router.push("/dashboard")`. Estados de *loading* via `useTransition`; alvos de toque ≥44px; sem `any`. **Página (`app/(app)/definicoes/nova-epoca/page.tsx`, novo — Server Component):** chama `verificarElegibilidadeWizard()` (gating de acesso no servidor: `CLUBE_EPOCAS`; cenário D vê `EstadoErro` em vez do wizard) e passa a elegibilidade ao componente cliente. **Entry point (`app/(app)/definicoes/epocas/page.tsx` + `components/definicoes/EpocasLista.tsx`):** botão «Assistente nova época» (ícone `Wand2`, `Link` para `/definicoes/nova-epoca`) no cabeçalho da lista de épocas, mostrado apenas quando o membro tem `CLUBE_EPOCAS` (calculado via `obterMembroAtual`); coexiste com o diálogo «Nova época» manual já existente. **Não toca em auth.** Verificação: `npm run typecheck` (0 erros), `npm run lint` (0 avisos), `npm run test` (1025/1025), `npm run build` (`next build` exit 0 com heap alargado, rota `/definicoes/nova-epoca` gerada — a etapa `prisma migrate deploy` não corre no sandbox por a BD Supabase estar inacessível, como nas entradas anteriores).
- **2026-08-16** — **Implementação da camada de servidor do wizard «Nova Época» (§8.21) — cenários A/B/C.** Materialização em código da especificação funcional de §8.21 (nenhuma alteração ao texto de §8.21, que já a descrevia). **Schemas (`lib/schemas/novaEpoca.ts`, novo):** `transicaoAtletaSchema` (`atletaId` cuid, `transitaParaNova` bool, `novoNumero?` 1-999 nullable), `transicaoPlantelSchema`, `novaEpocaStep1Schema` (nome ≤20, datas com `dataFim > dataInicio`, `escalaoIds` ≥1), `promocaoEscalaoSchema` (origem ≠ destino), `novaEpocaRolloverSchema` (passo 1 + `atletas[]` + `promocoes[]`, defaults `[]`) e `novoClubeSchema` (`nomeClube`, `corClube` #hex, `escalaoNome`, flags `importarExercicios`/`importarModelosTaticos`/`importarMetricas` default `true`); helpers puros `calcularIdade` e `deveSerPromovido`. **Server Actions (`lib/actions/novaEpoca.ts`, novo):** `verificarElegibilidadeWizard()` devolve `{ cenario, atletasAtivos, escaloes }` (cenário `C` se licença INDIVIDUAL, senão `A`); `criarEpocaRollover()` (cenários A/B) cria a nova época **ativa** numa transação Serializable, transita cada atleta marcado com **uma** participação `AtletaEscalao` PRINCIPAL (invariante §9 — as promoções sobrepõem-se às transições regulares), **herda automaticamente** o conteúdo não filtrado por época (exercícios portáteis, métricas, habilidades, modo de semana) e **zera** estatísticas/presenças/jogos/convocatórias/planeamentos (não copiados; numeração de semanas recomeça em 1); `criarNovoClube()` (cenário C) cria Clube + Época ativa + Escalão + perfis de arranque + `MembroClube` Admin, encerra a adesão anterior (INATIVO) — recusando se o clube anterior tiver outros membros ativos (§9 «nunca deixar clube sem admin») — e transporta métricas/exercícios/modelos táticos do treinador conforme as flags; `sugerirPromocoes()` (cenário B) lista os atletas que excedem a `idadeMax` do escalão à data de início da época (sugestão, nunca automática). **Guarda de acesso (§8.21):** todas exigem a capacidade `CLUBE_EPOCAS` (Administrador ou Treinador Individual dono do clube); os perfis «Treinador Principal»/«Adjunto» adicionados por um DT não a têm, ficando de fora (cenário D). **Ativação da nova época** sincroniza o cookie `epoca_ativa` (seletor de época). **Testes:** `tests/novaEpoca.test.ts` (41 — schemas, helpers puros e as quatro actions com prisma mockado, incl. permissões, invariante do principal único, sobreposição da promoção, encerramento da adesão anterior e recusa a treinador adicionado por DT sem `CLUBE_EPOCAS`). **Não toca em auth.** Verificação: `npm run typecheck` (0 erros), `npm run test` (1025/1025), `npm run build` (compilação + type-check OK; `next build` exit 0 com heap alargado — a etapa `prisma migrate deploy` não corre no sandbox por a BD Supabase estar inacessível, como nas entradas anteriores).
- **2026-08-16** — **Implementação do mecanismo de snapshot §4.2.1 (`SessaoExercicio.snap*`).** Materialização em código da decisão de produto (D) do dia (ver entrada abaixo). **Schema (§3.5):** adicionados os campos `snapNome`/`snapDescricao`/`snapObjetivo`/`snapDiagrama`/`snapCriadoEm` ao modelo `SessaoExercicio` em `prisma/schema.prisma` (deixam de ser ⚠️ «alvo de spec» — passam a implementados). **Tipo do `snapDiagrama`:** `Json?` (JSONB), cópia fiel do `Exercicio.diagrama` (um `DiagramaCampo`, §11) — conforme especificado em §3.5. **Migração:** `prisma/migrations/20260816120000_add_sessao_exercicio_snapshot/migration.sql` (ALTER TABLE ADD COLUMN, aditivo, sem perda de dados). **Escrita (§4.2.1):** ao adicionar um exercício a uma sessão do clube — em `adicionarExercicioSessao` (`lib/actions/treinos.ts`) e em `criarSessaoDeTemplate` (`lib/actions/templatesSessao.ts`) — se `exercicio.proprietario = TREINADOR` os campos `snap*` são preenchidos automaticamente com o estado atual (nome, descrição, objetivo, diagrama) e `snapCriadoEm`; se `proprietario = CLUBE`, ficam a null. O snapshot é criado **uma vez** e é **imutável** (nunca reescrito em edições posteriores). **Leitura:** helper puro `resolverExercicioSessao` (`lib/snapshot-exercicio.ts`) com fallback `exercicio?.nome ?? snapNome ?? "(exercício removido)"`, aplicado no detalhe do treino (`app/(app)/treinos/[id]/page.tsx`) — o histórico do clube reconstrói-se mesmo depois de o treinador sair. **Testes:** `tests/snapshot-exercicio.test.ts` (funções puras `construirSnapshotExercicio` + `resolverExercicioSessao`). **Não toca em auth.** Verificação: `npm run typecheck`, `npm run build`, `npm run test`.
...
- **2026-08-13** — **F3.3 — Aviso não-bloqueante de conflito de pavilhão em `SessaoForm`/`JogoForm` (§8.16; `components/treinos/SessaoForm.tsx`, `components/jogos/JogoForm.tsx`).** Fatia de **UI** da feature F3 do `docs/ROADMAP_EXECUCAO.md` (deteção de conflitos de pavilhão), sobre o backend já existente (F3.1 `lib/utils/agenda-conflitos.ts` e F3.2 `lib/actions/agenda.ts::verificarConflitoAgenda`, **não tocados**). Ambos os formulários passam a chamar `verificarConflitoAgenda` **após-debounce (~800ms via `setTimeout`/`clearTimeout` no cleanup do `useEffect`)** quando os campos relevantes estão preenchidos: `SessaoForm` com `tipo: "TREINO"`, `data`, `local`, `duracaoMin` e `excluirId: sessao?.id`; `JogoForm` com `tipo: "JOGO"`, `data`, `local`, `duracaoMin: undefined` (jogos não têm duração → o backend assume `DURACAO_PADRAO_MIN = 90` min) e `excluirId: jogo?.id`. Havendo sobreposição, mostra-se um **banner âmbar não-bloqueante** (`role="alert"`, `bg-amber-50`/`border-amber-300`/`text-amber-800` — âmbar, não vermelho, distinto de erro de validação) intitulado "Possível conflito de pavilhão", com uma lista de "{Treino|Jogo} do {escalão} — {HH:mm}" (hora via `date-fns` `format` + locale `pt`), colocado imediatamente antes dos botões de submissão. **Nunca impede guardar.** Guardas: sem `local` (ou sem `data`/`escalaoId`) não verifica (`setConflitos([])`); em falha de rede/verificação, **silêncio total** (`try/catch` → sem conflitos, sem erro visível — funcionalidade secundária). Para captar os valores dos campos não-controlados sem alterar a submissão (que continua a ler via `FormData`), adicionaram-se `onChange` que espelham `data`/`duracaoMin`/`local` (Sessão) e `local` (Jogo) em estado local; o `JogoForm` já tinha `data` e `escalaoId` em estado. Flag `cancelado` no cleanup evita atualizações de estado obsoletas de promessas em voo. **Sem alteração ao backend, schema Prisma, base de dados, permissões ou auth.** Verificação: `npm run typecheck`, `npm run lint`, `npm run test`.

- **2026-08-13** — **F1.1 + F1.2 — Export CSV dos analíticos: utilitário puro + Server Actions (§8.15; `lib/utils/csv.ts` (novo), `lib/schemas/analise.ts`, `lib/actions/analise.ts`, `tests/csv.test.ts` (novo), `tests/export-csv.test.ts` (novo)).** Primeira fatia da feature F1 do `docs/ROADMAP_EXECUCAO.md` (export CSV/PDF dos analíticos) — apenas o **backend** (o botão de download na UI é F1.3, a cargo do `frontend-specialist`; a via de PDF é F1.4). **F1.1 — helper puro** `lib/utils/csv.ts`: `paraCsv(linhas, colunas)` serializa um conjunto de linhas num CSV para **Excel PT-PT** — **BOM UTF-8** (acentos), separador **`;`**, **escape RFC 4180** (campos com `;`/`"`/quebra de linha envolvidos em aspas, aspas internas duplicadas), terminador **CRLF**, `null`/`undefined` → célula vazia, e números via `String()` (**ponto decimal**, nunca vírgula, para o Excel os ler como número). Inclui `juntarBlocosCsv(...blocos)` (junta tabela + resumo mantendo um único BOM) e a constante `BOM_UTF8`. Sem dependências externas, síncrono, sem I/O (mesma filosofia de `lib/utils/cargaTreino.ts`). **F1.2 — Server Actions** `exportarAnaliticoEscalaoCsv({ escalaoId, competicaoId? })` e `exportarAnaliticoAtletaCsv({ atletaId, escalaoId })` em `lib/actions/analise.ts` (schemas `exportarEscalaoCsvSchema`/`exportarAtletaCsvSchema` em `lib/schemas/analise.ts`, cuid). **Zero recálculo:** cada action delega em `obterAnaliticoEscalao`/`obterAnaliticoAtleta` — que já garantem `auth()` + capacidade `RELATORIOS_VER` + `podeLerEscalao` (e, no atleta, que participa no escalão) — e **serializa a estrutura devolvida**, pelo que os números batem por construção com os dos painéis (Regra Nº 6 — a fonte é a estrutura analítica, não um cálculo paralelo). Devolvem `Resultado<{ csv: string; nomeFicheiro: string }>` com `nomeFicheiro` slugificado (`analitico-{escalão|atleta}-AAAA-MM-DD.csv`). **Escalão:** tabela por atleta (nome, golos, assistências, jogos utilizados, minutos acumulados + 1 coluna por métrica configurável, unindo os rankings de `AnaliticoEscalao` por `atletaId`) + bloco de resumo (jogos, V/E/D, golos, médias, sessões, atletas, **taxa de presença média** — que em `AnaliticoEscalao` é métrica de equipa, não por atleta, logo vai ao resumo). **Atleta:** evolução jogo a jogo (data pt-PT, adversário, utilizado, golos, assistências + defesas/golos sofridos se GR) terminada por linha de **totais**, + bloco de resumo da época (jogos convocado/utilizados, titularidades, **minutos acumulados**, **taxa de presença**, métricas configuráveis agregadas — detalhes que `AnaliticoAtleta` só expõe agregados, não por jogo). **Testes** (24 novos): `tests/csv.test.ts` (14) cobre BOM, separador, CRLF, acentos, nulos, ponto decimal, escape RFC 4180 e `juntarBlocosCsv`; `tests/export-csv.test.ts` (10) cobre permissão negada, escalão inexistente, atleta que não participa, input inválido, colunas de GR condicionais, coluna dinâmica de métrica, e o cabeçalho/linhas/resumo do CSV resultante. **Sem alteração ao schema Prisma, base de dados, permissões ou auth. `PainelEscalao.tsx`/`PainelAtleta.tsx` não foram tocados** (integração da UI fica para F1.3). Verificação: `npm run typecheck` **0 erros**; `npm run lint` **0 erros/avisos**; `npm run test` **961/961 a passar**.
...
- **2026-08-13** — **Fix visual da landing pública: fundo branco forçado independentemente do tema do sistema (`app/page.tsx`, `app/globals.css`).** A landing abrindo com fundo preto/muito escuro e texto ilegível em qualquer contexto em que o tema escuro estava ativo (default do produto). Causa raiz: dois bugs de especificidade CSS em simultâneo. (1) O elemento `<main class="landing-root bg-white">` com ambas as classes no mesmo elemento não era abrangido pelo seletor descendente `.dark .landing-root .bg-white` — esse combinador exige relação pai/filho, pelo que `.dark .bg-white` (especificidade 0,2,0) vencia `.landing-root` (0,1,0) e o fundo ficava `#1c1b22`. (2) O seletor de tipo `.dark h2, .dark h3` (0,1,1) sobrepunha-se a `.text-cinza-900` (0,1,0), tornando os títulos das secções claras cor creme sobre fundo escuro. **Fix duplo:** em `app/page.tsx`, removidas as classes `bg-white` e `text-cinza-900` do `<main>` e substituídas por `style={{ backgroundColor: "#ffffff", color: "#141210" }}` (estilos inline batem qualquer regra CSS sem `!important`); em `app/globals.css`, o bloco `.landing-root` passa a usar `html .landing-root, html.dark .landing-root { background-color: #ffffff !important; color: #141210 !important; }` (especificidade 0,2,1 + `!important`) e `html.dark .landing-root h1/h2/h3 { color: inherit; }` (0,2,2, vence 0,1,1) para que os títulos herdem a cor do contexto — branco no hero (secção com `text-white`), `#141210` nas secções claras. **Sem alteração de schema, actions, auth ou testes.** Verificação: `npm run typecheck` 0 erros; `npm run lint` 0 erros/avisos; `npm run test` 870/870 a passar.

- **2026-08-13** — **Fix de build: funções puras de carga de treino extraídas para fora do módulo de Server Actions (§8.20; `lib/utils/cargaTreino.ts` (novo), `lib/actions/cargaTreino.ts`, `components/graficos/CurvaCargaSemanal.tsx`, `tests/cargaTreino.test.ts`).** O build de produção falhava com `Server Actions must be async functions` porque `lib/actions/cargaTreino.ts` tem `"use server"` no topo — um módulo com essa diretiva só pode exportar funções `async`, e o ficheiro exportava também os helpers **síncronos** `calcularCargaSemanal`, `classificarAcwr` e `inicioSemana` (e a constante `LABEL_ZONA_CARGA`). Esses símbolos passam para o novo módulo **`lib/utils/cargaTreino.ts`** (sem `"use server"`), a par dos tipos `ZonaCarga`, `DadosCargaSemanal`, `SessaoCarga` e da constante `MS_SEMANA`. `lib/actions/cargaTreino.ts` importa agora o cálculo desse módulo e exporta **apenas** as três Server Actions `async` (`registarRpeSessao`, `registarRpeAtleta`, `obterCargaSemanal`) e o tipo `CargaSemanalEscalao`. `CurvaCargaSemanal` e `tests/cargaTreino.test.ts` passam a importar as funções puras/tipos de `@/lib/utils/cargaTreino` (o teste mantém os mocks e os imports das Server Actions de `@/lib/actions/cargaTreino`). **Sem alteração de comportamento, de schema, de permissões ou de UI** — apenas relocalização de código. Build de produção verde, 870 testes verdes. **Não toca em auth.**

- **2026-08-12** — **Testes de unidade para actions sem cobertura (P3.1) (`tests/jogos.test.ts` (novo), `tests/escaloes.test.ts` (novo), `tests/epocas.test.ts` (novo), `tests/atletas.test.ts` (novo), `tests/metricas.test.ts` (novo), `tests/habilidades.test.ts` (novo), `tests/caderneta.test.ts` (novo), `tests/periodizacao.test.ts` (novo), `tests/reunioes.test.ts` (novo)).** Cobertura das Server Actions sem testes unitários, usando o padrão Vitest + mocks Prisma do projeto. **Sem qualquer alteração a código de produção, schemas Prisma ou base de dados; não toca em auth.** 9 ficheiros novos, 195 testes adicionais (de 675 para 870 no total). Actions cobertas: **`jogos.ts`** (`criarJogo`, `atualizarJogo`, `apagarJogo`, `guardarRelatorio`, `definirVideo`, `registarEventoJogo`, `removerEventoJogo`) — validação da allowlist YouTube https, isolamento multi-tenant (escalão/competição pertence ao clube), guard de auth (`auth()`/`obterClubeIdAtual`), verificação dos campos `epocaId`/`criadorId` na criação, e trim + null no relatório; **`escaloes.ts`** (`listarEscaloes`, `criarEscalao`, `atualizarEscalao`, `definirVisibilidadeEscalao`, `moverEscalao`) — guarda `obterClubeIdAtual`, validação Zod (nome vazio, idadeMax < idadeMin, nome > 50 chars), isolamento por clube, cálculo de `ordem` incremental e troca de ordens em transação; **`epocas.ts`** (`listarEpocas`, `criarEpoca`, `definirEpocaAtiva`, `selecionarEpoca`) — validação de `dataFim > dataInicio`, transação `updateMany`+`update` para mudança de época ativa, isolamento por clube; **`atletas.ts`** (`apagarAtletaDefinitivamente`, `obterEstatisticasAtleta`) — guard de cuid (schema), bloqueio com estatísticas (proteção dados desportivos RGPD), aggregação com `totalGolos`/`taxaPresenca` como ratio 0-1, casos edge zero-jogos e 100% presença; **`metricas.ts`** (`listarMetricas`, `criarMetrica`, `alternarMetrica`, `moverMetrica`) — filtro `apenasAtivas`, cálculo de `ordem`, tipo inválido, isolamento; **`habilidades.ts`** (`listarHabilidades`, `criarHabilidade`, `atualizarHabilidade`, `moverHabilidade`) — reordenação dentro do mesmo nível, nível inválido; **`caderneta.ts`** (`obterCadernetaAtleta`, `atualizarProgresso`) — merge de progressos, `dataDesbloqueio` ao DESBLOQUEADO/null ao regredir, notes opcionais; **`periodizacao.ts`** (`criarPlaneamento`, `sugerirPlaneamento`, `apagarPlaneamento`) — inferência de período PREPARATORIO/TRANSICAO por percentagem da época, incremento de microciclo, duração SEMANAL=6/MENSAL=27 dias, `dataFim >= dataInicio`; **`reunioes.ts`** (`criarReuniao`, `atualizarReuniao`, `apagarReuniao`) — reunião de escalão exige `escalaoId`, `criadorId` correto. `apagarEscalao` (já em `tests/actions-producao.test.ts`), `apagarHabilidade` (idem), `criarAtleta`/`apagarAtleta` (já em `tests/actions.test.ts`) e `definirConvocatoria`/`guardarEstatisticas` (idem) não foram duplicados. Verificação: `npm run typecheck` **0 erros**; `npm run lint` **0 erros/0 avisos**; `npm run test` **870/870 a passar**.

- **2026-08-12** — **Cards sociais nativos para Instagram: resultado, MVP e ranking (P4.7) (§3.16; `lib/schemas/social.ts` (novo), `lib/social/token.ts` (novo), `lib/social/card-data.ts` (novo), `app/api/social/card/route.tsx` (novo), `components/social/BotoesPartilhaJogo.tsx` + `BotaoPartilhaRanking.tsx` (novos), `app/(app)/jogos/[id]/page.tsx`, `app/(app)/escaloes/[id]/analiticos/page.tsx`, `tests/social-card.test.ts` (novo)).** Fecha o loop de conteúdo orgânico (UGC): a app passa a **gerar imagens quadradas 1080×1080 PNG** prontas para Instagram, com emblema/cor do clube e marca Mister discreta. **Sem qualquer alteração ao schema Prisma.** Três tipos de card: **`resultado`** (placar Clube vs Adversário + data · escalão · competição), **`mvp`** (jogador em destaque eleito por pontuação sobre as `EstatisticaAtleta` do jogo — golo=3, assistência=2, defesa=0,5; desempate por golos e depois assistências — com golos/assistências/defesas se GR) e **`ranking`** (Top 5 marcadores do escalão/época, agregado por `atletaId` para não fundir homónimos, §10.2). **Rota** `GET /api/social/card` (runtime Node, `dynamic = "force-dynamic"`) desenha o card com `ImageResponse` de **`next/og`** (já disponível no Next 15.5); parâmetros `tipo` + `jogoId` (resultado/mvp) ou `escalaoId`+`epocaId` (ranking) + `token`. **Agregação** em `lib/social/card-data.ts` (`server-only`): Prisma com **scoping ao clube autenticado** (`obterClubeIdAtual`) e verificação de leitura do escalão (`podeLerEscalao`), devolvendo `RespostaCard<T>` com `status` HTTP (401/403/404). **Token anti-scraping** (`lib/social/token.ts`): HMAC-SHA256 determinístico do recurso (`gerarTokenCard`/`validarTokenCard`/`urlCard`, comparação `timingSafeEqual`), gerado no servidor e embutido no URL de partilha; usa segredo dedicado `SOCIAL_CARD_SECRET` (degradação graciosa em dev) — **não usa segredos de autenticação nem toca em auth**. **Botões:** «Partilhar resultado» (só com resultado) e «Ver MVP» em `/jogos/[id]`; «Partilhar ranking» em `/escaloes/[id]/analiticos`; abrem a imagem numa nova aba (`<a target="_blank">`), sem download automático. **RGPD (§3.14, regra dura):** os cards **nunca** expõem menores — escalões de formação jovem (sub-14 e inferiores, com deteção tolerante a grafias "Sub-12"/"sub 12"/"SUB12", + petizes/traquinas/benjamins/escolas/minis) são bloqueados com **`403`** «Cards sociais não disponíveis para escalões de formação jovem (RGPD)» e os botões **não são sequer renderizados** para esses escalões (dupla proteção servidor+UI). `tests/social-card.test.ts` (17): `cardQuerySchema` (exige jogoId/escalaoId+epocaId conforme o tipo, rejeita sem token/tipo inválido), `eEscalaoFormacaoJovem` (bloqueia sub-≤14 e categorias jovens, permite sub-15+/seniores) e os tokens (determinismo, tokens distintos por recurso, validação correta/errada, `urlCard`). **Não toca em auth.**
- **2026-08-12** — **Análise de carga de treino: RPE / ACWR (P4.8) (§8.20; campo `Sessao.rpeSessao` + model `RpeAtleta` (novos); `lib/schemas/cargaTreino.ts` (novo), `lib/actions/cargaTreino.ts` (novo), `components/graficos/CurvaCargaSemanal.tsx` (novo), `components/treinos/RegistoRpeSessao.tsx` (novo), `app/(app)/treinos/[id]/page.tsx`, `app/(app)/escaloes/[id]/analiticos/page.tsx`, `tests/cargaTreino.test.ts` (novo)).** Introduz monitorização de carga percebida (sRPE) e do rácio agudo:crónico (ACWR) por escalão — ferramenta de prevenção de lesões e periodização baseada em evidência. **Schema:** novo campo opcional **`Sessao.rpeSessao`** (Int 1-10, RPE da sessão atribuído pelo treinador na escala CR10) e novo model **`RpeAtleta`** (RPE individual reportado por atleta/sessão, `@@unique([sessaoId, atletaId])`, Cascade em ambas as FKs). Migração SQL manual `20260812150000_add_carga_treino` (BD Supabase inacessível no ambiente; aplicar em produção com `db:deploy`). **Schemas Zod** (`lib/schemas/cargaTreino.ts`): `registarRpeSchema` (sessaoId cuid + rpe int 1-10) e `registarRpeSessaoSchema` (sessaoId cuid + rpeSessao int 1-10). **Server Actions** (`lib/actions/cargaTreino.ts`): `registarRpeSessao(sessaoId, rpeSessao)` — exige `TREINOS_GERIR` no escalão da sessão, atualiza `Sessao.rpeSessao`; `registarRpeAtleta(sessaoId, atletaId, rpe)` — exige auth + sessão/atleta do clube, upsert em `RpeAtleta`; `obterCargaSemanal(escalaoId, semanas=8)` — exige `RELATORIOS_VER` + leitura do escalão, devolve por semana a carga acumulada (Σ duracaoMin × rpeSessao), o RPE médio e o **ACWR** (carga da semana / média das até 4 semanas anteriores). Cálculo isolado na função **pura e testável** `calcularCargaSemanal(sessoes, semanas, agora)` + helpers `inicioSemana` (segunda-feira ISO) e `classificarAcwr`. **Zonas de ACWR:** `< 0.8` subcarga · `0.8–1.3` zona ideal · `> 1.3` risco de sobrecarga. **UI:** `RegistoRpeSessao` (cliente — seletor 1-10 CR10, alvos ≥44px) na página do treino após presenças; `CurvaCargaSemanal` (gráfico SVG próprio de barras de carga com código de cores por zona + linha ACWR sobreposta e banda da zona ideal) na secção «Carga de treino» dos analíticos do escalão, mostrada só quando há RPE registado (`temDados`). `tests/cargaTreino.test.ts` (29): schemas (1-10, rejeita 0/11/não-inteiro/string), `classificarAcwr` nas fronteiras, `inicioSemana`, `calcularCargaSemanal` (carga, RPE médio, ACWR risco/ideal/subcarga, null sem histórico, ignora sessões sem RPE) e as três Server Actions (permissões, upsert, ACWR via mock Prisma). **Não toca em auth.**
- **2026-08-12** — **Decisão documentada: escrita concorrente / audit log — last-write-wins aceite para o MVP (P3.3) (§13.4).** Task de **decisão documentada** (sem qualquer alteração de código, schema Prisma ou base de dados; não toca em auth). A auditoria identificou que as server actions fazem *upsert* **sem deteção de conflito** — dois utilizadores a editar o mesmo registo em simultâneo resultam em *last-write-wins* silencioso — e que a maioria dos modelos não tem `criadoPor`/`atualizadoPor` nem optimistic locking. **Decisão do supervisor: aceitar *last-write-wins* como comportamento documentado para o MVP**, com base em (1) perfil de utilização essencialmente monoutilizador por escalão (um treinador + eventual adjunto), tornando a edição simultânea do mesmo registo um cenário raro; (2) o custo O(n) de introduzir `version`/optimistic locking em todas as entities não se justifica para o MVP; (3) as operações mais críticas (grelha de estatísticas de jogo) têm latência baixa, minimizando a janela real de conflito; (4) a decisão é revisitável quando houver dados de usage que confirmem o problema. Documentado em **§13.4** como limitação não-funcional conhecida, incluindo o trabalho **futuro** deixado pendente (campo `version` + verificação de `updatedAt` antes de escrever em `EstatisticaAtleta`/`Sessao`/`Jogo`, e opcionalmente `criadoPor`/`atualizadoPor` nesses modelos de alta escrita). **Zero alterações de código; apenas documentação.**
- **2026-08-12** — **Enriquecimento do perfil do treinador: métricas de carreira + copiar link (P4.5) (§8.17; `lib/actions/perfis.ts` (`obterResumoCarreira`, tipo `ResumoCarreira`), `app/(app)/perfil/page.tsx`, `components/perfil/CopiarLinkPerfil.tsx` (novo), `tests/perfis.test.ts`).** Evolução sobre P2.4, **sem qualquer alteração ao schema Prisma** (zero colunas/migrações novas). (1) **Métricas agregadas** — nova Server Action `obterResumoCarreira` (verifica `auth()`, filtra por `utilizadorId` do utilizador autenticado) que calcula em JS sobre o `findMany` (sem SQL de agregação) o tipo `ResumoCarreira`: `totalRegistos`, `clubesDistintos` (nomes únicos, case-insensitive após trim), `epocasAtivas` (registos sem `epocaFim`), `conquistasTotal` (registos com `conquistas` não vazias) e `primeiraEpoca` (`epocaInicio` mais antiga, ou null). (2) **Painel de métricas** em `/perfil`: três cards `grid-cols-3` antes da lista — "X épocas em Y clubes", "Z conquistas", "Desde AAAA/AAAA" — com tokens suaves da marca (`bg-cinza-50`/`bg-laranja-50`), número grande + label; só aparecem quando há registos. (3) **Copiar link do perfil** (`CopiarLinkPerfil.tsx`, cliente): botão no cabeçalho que copia `${origin}/perfil/{userId}` via `navigator.clipboard.writeText()` + toast "Link copiado!". **Deliberadamente sem coluna `partilhaPublica` nem rota nova** — a partilha pública propriamente dita continua FUTURO (requer mais design). `tests/perfis.test.ts` reforçado (+3): `obterResumoCarreira` sem sessão, sem registos (zeros) e com 3 registos (2 clubes, 1 conquista, 1 em curso → valores corretos). **Não toca em auth.**
- **2026-08-12** — **Arranque de clube utilizável: semear época ativa + escalão e acionar o wizard de onboarding (P1.6+P1.7) (§8.1, §16 fase 20; `lib/actions/onboarding.ts` (`criarClube`), `components/onboarding/CriarClubeForm.tsx`, `tests/onboarding.test.ts` (novo)).** Fecha o beco em que um clube recém-criado ficava **sem época e sem escalão** → `obterEpocaAtiva()` devolvia `null`, o dashboard bloqueava em «Nenhuma época ativa» e o percurso de vitória rápida arrancava sem contexto. **P1.6:** dentro da transação de `criarClube()`, **após criar o `Clube`**, passam a ser semeados (1) uma **`Epoca` corrente ativa** — nome no formato desportivo `"AAAA/AAAA"` derivado do mês corrente (a época de futsal arranca em setembro; antes de agosto ainda se está na época iniciada no ano anterior), `dataInicio` a 1 de setembro, `dataFim` a 30 de junho e `ativa: true` para que `obterEpocaAtiva()` deixe de devolver `null`; e (2) um **`Escalao`-semente editável** «Seniores» com `ordem: 1` e `visivelOutrosTreinadores: true`, customizável depois no wizard. Ambos partilham a mesma transação do clube/perfis/membro (atomicidade — ou tudo, ou nada). **P1.7:** o wizard `app/(onboarding)/` existia mas **nunca era acionado**; `CriarClubeForm` passa a redirecionar para **`/onboarding`** após `criarClube()` com sucesso (`router.push` no cliente, mantendo o contrato `Resultado<T>` da action e o `toast.success` — um `redirect()` dentro da action quebraria o retorno do resultado). A página de onboarding já protege contra reentrada (`onboardingConcluido`). **Não toca em auth.** Sem alterações ao schema Prisma (campos `Epoca.ativa`, `Escalao.ordem`/`Escalao.visivelOutrosTreinadores` já existiam). Novo `tests/onboarding.test.ts` (7) valida, com mocks Prisma no padrão do projeto, que `criarClube()` cria uma época ativa (`ativa: true`, nome `"AAAA/AAAA"`, `dataInicio < dataFim`) e o escalão-semente («Seniores», `ordem: 1`, `visivelOutrosTreinadores: true`) na mesma transação, que o membro administrador continua a ser criado (regressão do fluxo base), e que sessão inválida / adesão ativa prévia / input inválido não tocam na base de dados.
- **2026-08-12** — **Perfil do treinador / histórico de carreira (P2.4) (§8.17; model `RegistoCarreira` (novo); `lib/schemas/perfil.ts` (novo), `lib/actions/perfis.ts`, `app/(app)/perfil/page.tsx` (novo), `components/perfil/ListaRegistosCarreira.tsx` + `RegistoCarreiraForm.tsx` + `RegistoCarreiraAcoes.tsx` + `NovoRegistoCarreira.tsx` (novos), `components/layout/BarraTopo.tsx`, `tests/perfis.test.ts` (novo)).** Diferenciador-chave da licença Individual (§17.3, "o que crias é teu para toda a carreira"): perfil profissional do treinador com **histórico de carreira** editável. Novo model **`RegistoCarreira`** — passagem por um clube/escalão com `clube` (texto livre — pode ter treinado fora do sistema), `escalao`, `epocaInicio`, `epocaFim` (null = em curso), `conquistas`, `notas` e `ordem`. **Pertence ao `Utilizador` (a pessoa), não ao clube** — é **portátil** e viaja com o treinador entre clubes (`onDelete: Cascade`); por isso as actions filtram por `utilizadorId` do utilizador autenticado e **não** por clube/época. Migração SQL manual `20260812140000_add_registo_carreira` (a aplicar em produção com `db:deploy`). Quatro Server Actions em `lib/actions/perfis.ts` (`obterRegistosCarreira`, `criarRegistoCarreira`, `atualizarRegistoCarreira`, `eliminarRegistoCarreira`): todas verificam `auth()` primeiro e as de escrita fazem **ownership check** (`utilizadorId === session.user.id`) antes de escrever; validação Zod (`lib/schemas/perfil.ts`); revalidam `/perfil`. UI em `/perfil` (Server Component com identidade nome+email + lista) acessível pelo menu do utilizador (`BarraTopo` → "Perfil e carreira", apenas novo `<Link>`, **sem tocar no sign-out/auth**). `tests/perfis.test.ts` (24): schemas (obrigatórios/opcionais/trim) + actions (filtro por utilizador autenticado, ordem incremental na criação, ownership em editar/eliminar, revalidação de `/perfil`). **Não toca em auth.**
- **2026-08-12** — **Simplificação do `JogoForm`: agendar vs registar resultado (P4.3) (§8; `components/jogos/JogoForm.tsx`, `lib/schemas/jogo.ts`, `lib/actions/jogos.ts`, `tests/schemas.test.ts`).** O formulário de jogo pedia **13 campos** — incluindo golos, faltas por parte e `videoUrl` — mesmo para **agendar um jogo futuro**, gerando carga cognitiva e confusão (ux-specialist). Reorganizado em dois grupos: (1) **«Agendar»** (sempre visível): data e hora, adversário, local (casa/fora), tipo (oficial/amigável), escalão, **competição** e recinto; (2) **«Resultado (opcional)»** (condicional): golos marcados/sofridos, faltas 1ª/2ª parte e `videoUrl`. O grupo de resultado só aparece quando o **jogo já aconteceu** (data no passado), **já tem resultado registado**, ou o treinador clica em **«Registar resultado»** (caso de jogo histórico) — assim, criar um jogo futuro mostra apenas os campos de agendamento. Removida a **duplicação «Competição»**: existiam dois campos com o mesmo rótulo — o `Select` de `competicaoId` e um `Input` de texto livre `competicao`; **mantém-se apenas o `Select`**. O campo legado `Jogo.competicao` (texto livre) foi **deprecado silenciosamente**: continua no modelo Prisma (sem migração) e é ainda **lido** para mostrar competições de jogos antigos (`app/(app)/jogos/page.tsx`, `[id]/page.tsx`, `lib/actions/comunicacao.ts`), mas **deixa de ser escrito** — em `criarJogo` fica a `null` e em `atualizarJogo` não é reescrito, **preservando** valores legados existentes. Removido também do `jogoSchema` (Zod), onde era *dead field*. Os campos de resultado já eram `.optional()`/`.nullable()` no schema, pelo que **agendar sem resultado** continua válido. **Sem alteração de schema Prisma nem base de dados; não toca em auth.** `tests/schemas.test.ts` reforçado (agendar sem resultado, jogo disputado com resultado completo, e o campo legado `competicao` ignorado pelo schema).
- **2026-08-12** — **UI das métricas configuráveis nos analíticos (P1.9) (§10.1, §10.2; `components/analiticos/PainelAtleta.tsx`, `PainelEscalao.tsx` + novo `RankingsMetricas.tsx`).** Fecha o ciclo das métricas custom: a **agregação server-side** (`obterAnaliticoAtleta → metricas`, `obterAnaliticoEscalao → rankingsMetricas`) já existia e estava testada (entrada de 2026-08-11), mas a **visualização ficara deferida** — os `ValorMetrica` continuavam a ser gravados por jogo (`JogoDetalhe.tsx`) sem nunca serem mostrados ao treinador. Esta task adiciona **apenas a camada de apresentação** (sem tocar em `lib/actions/analise.ts`, schema Prisma, actions ou base de dados; **não toca em auth**): (1) **`PainelAtleta`** ganha a secção **«Métricas personalizadas»** — por métrica: nome · total · média/jogo; estado vazio «Nenhuma métrica personalizada registada.» quando não há valores. (2) **`PainelEscalao`** ganha a secção **«Ranking por métrica»** via novo **`RankingsMetricas.tsx`** (client) — seletor `shadcn/ui` (`Select`) para escolher a métrica e lista ordenada dos melhores atletas; só renderiza quando `rankingsMetricas.length > 0`. (3) **Zero regressão na vista pública** — `PainelRelatorio → PainelAtleta/PainelEscalao` desenha snapshots imutáveis (§10.6); snapshots antigos (pré-agregação) não têm os campos, pelo que ambos os painéis usam defaults defensivos (`metricas = []`, `dados.rankingsMetricas ?? []`). A agregação e os seus testes (`tests/analise-f9.test.ts` — métricas vazias, agregação por atleta e rankings ordenados por valor) mantêm-se inalterados.
- **2026-08-12** — **Polish das presenças: marcar todos + barra de guardar fixa (P4.1+P4.2) (§8.5; `components/treinos/MarcadorPresencas.tsx`, `app/(app)/treinos/[id]/page.tsx`).** Melhorias de UX no marcador de presenças, sem alterar schema Prisma, actions nem base de dados; **não toca em auth**. (1) **P4.1 — atalhos client-side** acima da lista: botão **«Marcar todos presentes»** (define todos os atletas como `PRESENTE` e limpa motivos de falta) e botão **«Repor»** (descarta alterações não guardadas, voltando ao estado que veio da BD — ausência continua a assumir `PRESENTE` por defeito). Nenhum submete o formulário; são apenas clareza sobre o comportamento por defeito já existente. (2) **P4.2 — barra de guardar fixa**: o resumo (`X presentes · Y faltas`) e o botão **«Guardar presenças»** passam para uma barra `sticky bottom-0` sempre visível ao percorrer listas longas de atletas; o botão garante alvo de toque **≥44px** (`min-h-[44px]`) e ocupa a largura total em mobile. Não há duplicação — o botão original solto foi movido para a barra fixa. (3) **Ordem em mobile**: na página do treino, as **presenças passam a aparecer antes dos exercícios em ecrãs pequenos** (via `order-*` do Tailwind), por ser a tarefa mais frequente no arranque do treino; em desktop mantém-se exercícios à esquerda / presenças à direita.
- **2026-08-12** — **Touch targets a 44px em elementos interativos (P2.9) (§19.5; `components/plantel/AtletaForm.tsx`, `components/onboarding/WizardOnboarding.tsx`, `components/layout/BarraTopo.tsx`, `app/layout.tsx`).** Correção da regressão silenciosa dos alvos de toque mínimos de **44px** exigidos por §19.5, identificada por dois agentes independentes (ux-specialist + ui-design-reviewer). Alterações: (1) **chips de posição** no `AtletaForm` passam de `px-3 py-1.5` (~32px) para `inline-flex min-h-[44px] items-center px-4 py-1.5`; (2) **botões de remover escalão** no `WizardOnboarding` passam de `h-9 w-9` (36px) para `h-11 w-11` (44px); (3) **ícones interativos da barra de topo** (indicador de evento e botão do menu de utilizador) no `BarraTopo` passam de `h-9 w-9` (36px) para `h-11 w-11` (44px); (4) removido `maximumScale: 1` do viewport em `app/layout.tsx`, que bloqueava o pinch-zoom (má prática de acessibilidade). O `SelectTrigger` (`components/ui/select.tsx`) já usava `h-11` (44px), pelo que não necessitou de alteração. Apenas ajuste de dimensões de área tocável — sem redesign, schemas, actions ou base de dados; **não toca em auth**.

- **2026-08-12** — **`COMUNICACOES_GERIR` no perfil Treinador Principal (P1.8) (§6.6; `lib/permissoes-catalogo.ts`, `tests/permissoes.test.ts` (novo)).** O modelo de arranque **Treinador Principal** passa a incluir a capacidade **`COMUNICACOES_GERIR`**, garantindo que o botão **«Gerar convocatória»** (WhatsApp) em `/jogos/[id]` — gated por `podeComunicar = membro.capacidades.includes("COMUNICACOES_GERIR")` em `app/(app)/jogos/[id]/page.tsx` e pela action `exigirCapacidade("COMUNICACOES_GERIR")` em `lib/actions/comunicacao.ts` — aparece ao treinador de escalão. É o **caso de uso nº1 da semana** (comunicar a convocatória aos pais). A capacidade já existia no catálogo (§6, `CAPACIDADES`); esta task acrescenta-a ao template de perfil e atualiza a documentação (§6.6) que a omitia. **Não toca em auth.** Novo `tests/permissoes.test.ts` (5) valida a presença da capacidade nos perfis Administrador, Diretor Técnico e Treinador Principal e a sua ausência no Adjunto.

- **2026-08-12** — **Sistema de Lembretes/Tarefas persistido (P2.1) (§3.15/§8.19; models `Lembrete` + `LembreteDestinatario` (novos); `lib/schemas/lembretes.ts` (novo), `lib/actions/lembretes.ts` (novo), `lib/permissoes-catalogo.ts`, `components/lembretes/ListaLembretes.tsx` + `LembretesPainel.tsx` + `CriarLembreteForm.tsx` (novos), `app/(app)/dashboard/page.tsx`).** Feature de retenção e coordenação de equipa do tier Clube. Torna persistente o conceito de lembrete que até aqui só existia como aviso **efémero** derivado no dashboard (`lib/dashboard-lembretes.ts`, "treino/jogo hoje"). Um lembrete tem título, descrição e data-limite opcionais, é **criado por um utilizador** e opcionalmente **dirigido a destinatários** (membros do clube); cada destinatário tem estado `visto` (badge "novo"). **Não toca em auth.**
  - **Modelação — `Lembrete` e `LembreteDestinatario`:** a pessoa (criador/destinatário) referencia `Utilizador`, **não `Perfil`** — no Mister o `Perfil` é um pacote de capacidades (papel), pelo que criador e destinatário têm de ser pessoas para que o estado `visto` e o filtro "criados por mim / onde sou destinatário" façam sentido. `Lembrete.clubeId` → `Clube` (Cascade), `Lembrete.criadoPorId` → `Utilizador` (Cascade); `LembreteDestinatario` com `@@unique([lembreteId, utilizadorId])` e Cascade em ambas as FKs. Índices em `clubeId`, `criadoPorId` e `utilizadorId`. Migração `20260812130000_add_lembretes` (escrita manualmente — BD Supabase inacessível no ambiente de desenvolvimento, aplicar com `db:deploy`).
  - **Actions (`lib/actions/lembretes.ts`):** `obterLembretes()` (lista os do utilizador — criados por mim OU onde sou destinatário — scoped ao clube, com estado `visto`/`souCriador` calculado); `criarLembrete(input)` (requer `LEMBRETES_EQUIPA_GERIR`; valida que os destinatários são membros do clube — isolamento multi-tenant); `atualizarLembrete(input)` (título/descrição/data-limite/`concluido`; **só o criador**); `marcarVisto(id)` (marca o registo de destinatário do utilizador atual); `eliminarLembrete(id)` (**só o criador** + `LEMBRETES_EQUIPA_GERIR`). Todas seguem a convenção `Resultado<T>` + Zod + `revalidatePath("/dashboard")`.
  - **Capacidade `LEMBRETES_EQUIPA_GERIR`:** já existia no catálogo (§6) e nos perfis Administrador e Diretor Técnico; adicionada também ao **Treinador Principal**.
  - **UI (dashboard):** `ListaLembretes` (Server Component) obtém os pendentes (não concluídos), a lista de destinatários possíveis e se pode gerir, delegando ao `LembretesPainel` (cliente) a lista com badge "novo", botão "Feito" (o criador conclui; o destinatário confirma/`visto`), eliminar (criador) e o diálogo "Novo lembrete" (`CriarLembreteForm`). Estado vazio: "Sem lembretes pendentes".
  - **Testes:** `tests/lembretes.test.ts` (11) cobre `criarLembreteSchema`/`atualizarLembreteSchema`/`idLembreteSchema`.

- **2026-08-12** — **Agenda agregada de todos os escalões (P2.2) (§8.x; `lib/actions/agenda.ts` (novo), `app/(app)/agenda/page.tsx` (novo), `components/agenda/FiltroEscalaoAgenda.tsx` (novo), `components/layout/Navegacao.tsx`, `app/(app)/layout.tsx`).** Vista única, cronológica e filtrável por escalão dos treinos e jogos de **todos os escalões** do clube — para o Diretor Técnico/Admin acompanhar a atividade sem navegar escalão a escalão. É uma **leitura agregada**: combina os dados que já existem em `Sessao` e `Jogo`, **sem entidade nova, sem migração Prisma e sem tocar em auth**.
  - **Action `obterAgendaClube(filtros?: { escalaoId?; mes?; ano? })` — `lib/actions/agenda.ts` (nova):** valida sessão/época (`obterClubeIdAtual`/`obterEpocaAtiva`), resolve a **janela temporal** (mês completo quando `mes` 1–12 + `ano` válidos; caso contrário **próximos 30 dias** a partir do início de hoje) e busca em paralelo (`Promise.all`) `Sessao` e `Jogo` do clube na época/janela, com `escalao { nome }`. Respeita o **âmbito de leitura (§6.4)** com o mesmo padrão de `listarSessoes`/`listarJogos` (`escaloesLegiveis` → `escalaoId: { in }` para treinadores; `podeLerEscalao` quando se filtra por um escalão específico, devolvendo `[]` sem permissão). Combina e **ordena cronologicamente** (crescente; desempate estável por tipo). Devolve `Resultado<EventoAgenda[]>` com `{ id, tipo: "TREINO" | "JOGO", data, escalaoNome, titulo, local? }` — título = objetivo da sessão (ou rótulo pt-PT do `TipoSessao`: Treino/Treino aberto/Captação/Evento) ou `vs {adversario}` no jogo.
  - **Página `app/(app)/agenda/page.tsx` (nova):** Server Component que chama `obterAgendaClube()` + `listarEscaloes()` em paralelo; agrupa os eventos por dia (cabeçalho de data pt-PT) e lista cada evento com ícone por tipo (`Trophy` jogo / `CalendarCheck` treino), hora, distintivo do escalão, tipo e local, com ligação a `/jogos/[id]` ou `/treinos/[id]`. Estado vazio «Sem eventos agendados»; mobile-first. Filtro por escalão via **`components/agenda/FiltroEscalaoAgenda.tsx`** (client Select `shadcn/ui` que escreve `?escalaoId=<id>` na URL com `useTransition`, seguindo o padrão de `FiltroCompeticao` — «Todos os escalões» limpa o filtro).
  - **Navegação — `components/layout/Navegacao.tsx` + `app/(app)/layout.tsx`:** novo item **«Agenda»** (ícone `CalendarRange`, a seguir a «Jogos»), **condicional** ao papel: só visível a Admin/DT (`membro.ambito === "TODO_CLUBE"`, prop `mostrarAgenda` calculada no layout). Segue o mesmo mecanismo do atalho condicional «Começar» (prop `mostrarComecar`).
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **625/625 a passar** (novo `tests/agenda.test.ts`, 9 testes: guardas de sessão/época, combinação e ordenação cronológica treinos+jogos, rótulo por `TipoSessao` sem objetivo, filtro por escalão com/sem permissão, restrição a escalões legíveis, lista vazia sem legíveis, janela do mês por `mes`/`ano`).

- **2026-08-12** — **UI da visibilidade de escalão para outros treinadores (P2.8) (§6.4, §3 model `Escalao`; `lib/schemas/escalao.ts`, `lib/actions/escaloes.ts`, `components/definicoes/EscaloesLista.tsx`).** O campo `Escalao.visivelOutrosTreinadores` (default `true`) já existia no schema e já governava a leitura de escalões alheios (§6.4: um membro pode **ler** um escalão que não é seu se `visivelOutrosTreinadores = true`), mas **nunca fora exposto na UI** — era um *dead field* impossível de alterar pelos treinadores. Complementa o Calendário DT (P2.2): quando um escalão está marcado como visível, outros treinadores do clube veem os seus treinos e jogos no calendário unificado. **Sem alteração de schema Prisma nem base de dados; não toca em auth.**
  - **`escalaoSchema` (`lib/schemas/escalao.ts`):** novo campo **`visivelOutrosTreinadores: z.boolean().optional()`** no objeto partilhado por criar/atualizar. Opcional: em `criarEscalao` não é enviado (a BD aplica o default `true`); em `atualizarEscalao` é persistido via o `data: parsed.data` já existente (Prisma ignora `undefined`, não sobrescreve). Sem alteração às assinaturas das actions.
  - **UI — `EscaloesLista.tsx`:** o `EscalaoForm` partilhado ganha a prop `mostrarVisibilidade` (default `false`); o diálogo **Editar** passa `mostrarVisibilidade` e envia `visivelOutrosTreinadores`. O diálogo **Criar** mantém-se inalterado (novos escalões nascem visíveis por defeito). O toggle usa o `Switch` (shadcn/Radix) controlado por estado, com um `input` escondido a sincronizar o valor para o `FormData` (o Radix Switch não emite valor nativamente); *label* «Visível para outros treinadores do clube» + descrição «Permite que outros treinadores vejam os treinos e jogos deste escalão no calendário do clube». Restrito a quem edita definições de escalão — a página Definições › Escalões e a action `atualizarEscalao` já exigem a capacidade `CLUBE_ESCALOES` (Admin/DT).
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **616/616 a passar** (4 novos testes em `tests/schemas.test.ts`: campo opcional ausente, persiste `false`, persiste `true`, rejeita não-booleano).

- **2026-08-12** — **Filtro por competição nos analíticos de escalão (P2.5) (§10.2; `lib/actions/analise.ts`, `lib/schemas/analise.ts`, `components/analiticos/PainelEscalao.tsx` + novo `FiltroCompeticao.tsx`, `app/(app)/escaloes/[id]/analiticos/page.tsx`).** `Jogo.competicaoId`/`Competicao` já existiam no schema mas os analíticos misturavam campeonato, taça e particulares no mesmo agregado — obrigando os treinadores a separar contextos fora da app. Agora é possível filtrar por competição. **Sem alteração de schema/BD; não toca em auth.**
  - **`obterAnaliticoEscalao(escalaoId, epocaId?, competicaoId?)`:** novo 3.º parâmetro opcional (`competicaoId`, validado como `cuid` em `analiticoEscalaoSchema`). Quando presente, aplica-se um `filtroJogo = { epocaId, escalaoId, ...(competicaoId ? { competicaoId } : {}) }` às **4 consultas que passam por jogo** (`jogo`, `estatisticaAtleta`, `eventoJogo`, `valorMetrica`), afetando V/E/D, golos, marcadores/assistentes, mais utilizados, eventos por tipo e rankings de métricas. **Sessões e presenças mantêm-se globais** (treinos são transversais à competição). Sem `competicaoId` o comportamento é idêntico ao anterior («Todas»).
  - **Nova action `obterCompeticoesEscalao(escalaoId, epocaId?)` → `Resultado<CompeticaoOpcao[]>`** (`{ id, nome, tipo }`): devolve as competições do escalão/época **com pelo menos um jogo** (`where.jogos.some`), ordenadas por `tipo` e `nome`. Exige `RELATORIOS_VER` + `podeLerEscalao` (mesmas guardas do analítico). Novo `competicoesEscalaoSchema`.
  - **UI:** novo **`components/analiticos/FiltroCompeticao.tsx`** (client) — seletor `shadcn/ui` que escreve `?competicao=<id>` na URL (segue o padrão de `FiltrosBiblioteca`); a página (Server Component) volta a chamar `obterAnaliticoEscalao` com o `competicaoId`, com **estado de carregamento** via `useTransition`. «Todas as competições» limpa o parâmetro (defeito). `PainelEscalao` ganha props opcionais `competicoes?`/`competicaoId?` e só renderiza o filtro quando há competições — mantém-se **Server Component** e o snapshot de relatório partilhado (`PainelRelatorio` → `PainelEscalao` sem essas props) fica **byte-idêntico** (zero regressão). A página passa a receber `searchParams` e a buscar analítico + competições em paralelo.
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **612/612 a passar** (5 novos testes em `tests/analise-f9.test.ts`: filtro por competição aplicado a jogos/estatísticas/eventos e ausente nas presenças; comportamento por defeito sem filtro; `obterCompeticoesEscalao` devolve a lista, nega escalão inexistente e nega sem permissão).

- **2026-08-12** — **Aba «Carreira» no perfil do atleta (P2.3, §8.5).** Exposição só-de-leitura do percurso do atleta (épocas, escalões, número de camisola, estado e período) que já existia em `AtletaEscalao` mas nunca fora apresentado num percurso dedicado; complementa a aba «Participações» (esta focada na gestão: associar/transferir/terminar). **Não toca em auth**, schema Prisma nem base de dados.
  - **Action `obterCarreiraAtleta(atletaId)` — `lib/actions/participacoes.ts` (nova):** valida sessão/clube (`obterClubeIdAtual`), confirma que o atleta pertence ao clube e aplica o âmbito de leitura da secção 6.4 (`podeLerAlgumEscalao` sobre os escalões do percurso). Lê todos os `AtletaEscalao` do atleta com `escalao { nome }` e `epoca { nome, ativa }`, ordenados da época mais recente para a mais antiga (`epoca.dataInicio desc`, depois `dataInicio desc`). Devolve `Resultado<CarreiraEntry[]>` com `{ id, epocaNome, epocaAtiva, escalaoNome, numero, estado, dataIngresso, dataSaida }` (`dataIngresso`/`dataSaida` = `dataInicio`/`dataFim` da participação).
  - **Componente `components/plantel/CarreiraAtleta.tsx` (novo):** Server Component só-de-leitura que consome a action. Lista cada etapa (época + distintivo «Época ativa» quando `epocaAtiva`, escalão, número quando existe, estado via `BadgeEstadoParticipacao` e período formatado). Estado vazio «Sem historial de épocas anteriores» (`EstadoVazio`); erro de leitura apresentado inline.
  - **Perfil — `app/(app)/plantel/[id]/page.tsx`:** novo separador «Carreira» (entre «Participações» e «Dados») que renderiza `<CarreiraAtleta atletaId={a.id} />`.
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run lint` **0 erros/0 avisos**; `npm run test` **todos a passar** (5 novos testes desta task em `tests/participacoes.test.ts`: isolamento por clube, âmbito de leitura, mapeamento `dataIngresso`/`dataSaida` + marca da época ativa, percurso vazio).

- **2026-08-12** — **Apagamento definitivo de atleta (hard-delete RGPD) (P1.3) (model `Presenca`, `Convocatoria`, `EstatisticaAtleta`; `lib/actions/atletas.ts`; `app/(app)/plantel/[id]/page.tsx`).** Complemento ao *soft-delete* existente (`ativo=false`, "Arquivar"): novo apagamento **irreversível** que remove o atleta e todos os dados pessoais associados, para cumprimento do **direito ao apagamento (RGPD)** — relevante para dados de menores. **Não toca em auth.** Migração `20260812120000_add_atleta_cascade`.
  - **Cascatas de FK completadas:** as relações obrigatórias que referenciavam `Atleta` e tinham `ON DELETE RESTRICT` (implícito) — **`Presenca.atletaId`**, **`Convocatoria.atletaId`** e **`EstatisticaAtleta.atletaId`** — passam a **`onDelete: Cascade`**, sem o que o *hard-delete* seria bloqueado. As restantes relações já tinham Cascade (`Consentimento`, `AtletaEscalao`, `EventoJogo`, `ProgressoHabilidade`); **`ValorMetrica`** cascateia transitivamente via `EstatisticaAtleta`. (Nota: `Convocatoria` não constava do enunciado da task mas referencia `atletaId` e bloquearia o apagamento, pelo que foi incluída.)
  - **Action `apagarAtletaDefinitivamente(atletaId)`** (`lib/actions/atletas.ts`): valida o `atletaId` com Zod (`apagarAtletaDefinitivamenteSchema`, **cuid** — a convenção de IDs do projeto; `uuid` rejeitaria todos os IDs reais); verifica `obterClubeIdAtual()` e que o atleta pertence ao clube; exige a capacidade **`PLANTEL_GERIR`** (`exigirCapacidadeEmAlgumEscalao` sobre os escalões das participações); **recusa** o apagamento quando existem estatísticas de jogo registadas (erro «Atleta com estatísticas registadas — exportar dados antes de apagar»); executa `prisma.atleta.delete` (os cascades limpam o resto); devolve `Resultado<void>` e revalida `/plantel`, `/plantel/[id]` e `/dashboard`.
  - **UI:** novo `components/plantel/ApagarAtletaDefinitivamenteButton.tsx` (client, `AlertDialog` de confirmação com aviso de irreversibilidade e menção ao consentimento do encarregado de educação; ao sucesso `router.push("/plantel")`). Integrado na aba **Dados** do perfil do atleta numa secção **"Zona de perigo"**, visível apenas a quem tem `PLANTEL_GERIR`.
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **602/602 a passar** (comportamento de apagamento coberto pelas cascatas do schema). Migração criada em `prisma/migrations/20260812120000_add_atleta_cascade/` (aplicação via `prisma migrate deploy` — a BD Supabase remota não está acessível a partir do ambiente de desenvolvimento sandbox).

- **2026-08-12** — **Contraste WCAG AA de texto branco sobre o laranja primário da marca (P2.7) (§12, §12.4).** Texto branco sobre `#F0531E` (laranja da marca) tem rácio de contraste **3.51:1**, abaixo do mínimo **WCAG AA (4.5:1)**. Correção pontual sem redesign nem alteração de estrutura, schemas, actions ou base de dados; **não toca em auth**. Regra aplicada: **fundos com texto branco** (botões/CTAs/badges) passam a usar **`laranja-600` (`#C7430F`, rácio 4.95:1)**; o laranja da marca (`#F0531E`/`primary`) mantém-se em **acentos, ícones, bordas e decorações sem texto por cima**.
  - **`components/ui/button.tsx` — variante `default`:** `bg-primary text-white hover:bg-primary/90` → **`bg-laranja-600 text-white hover:bg-[#A8370C]`**. O fundo do botão de ação principal passa a ser o laranja-600 fixo da marca (não o `--primary` dinâmico), garantindo AA no *default* de marca; o hover usa um laranja mais escuro (`#A8370C`) para maior contraste e coerência.
  - **`app/page.tsx` (landing pública) — 4 superfícies com texto branco corrigidas** de `#F0531E` → `#C7430F` (nova constante `LARANJA_ACAO`): CTA "Começar agora" no cabeçalho, CTA "Começar agora" do *hero*, *badge* "Recomendado" no plano em destaque, e CTA do plano em destaque. **Mantidos** em `#F0531E` (constante `LARANJA`, sem texto branco por cima): destaque de texto no `<h1>`, *chip* de ícone das funcionalidades, borda do plano em destaque, e ícones `Check` da lista de itens.
  - **Nota (fora do âmbito de P2.7):** outras 4 ocorrências de `bg-primary text-white` (toggle lista/calendário em `treinos`, *chip* de evento em `CalendarioTreinos`, *avatar* de iniciais em `UtilizadoresLista`, indicador de passo em `WizardOnboarding`) usam o `--primary` **dinâmico** (cor do clube) — só apresentam contraste insuficiente quando a cor do clube é clara. Alterá-las para `laranja-600` fixo quebraria o *branding* dinâmico do clube; ficam para decisão/tarefa própria de contraste da cor do clube.
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **602/602 a passar** (sem novos testes — alterações puramente de apresentação/CSS).

- **2026-08-12** — **Integridade referencial da BD: validação do índice de `AtletaEscalao` (P1.4) e constraints de FK em falta (P1.5) (model `AtletaEscalao`, `EventoJogo`, `Planeamento`, `Competicao`, `ObservacaoAdversario`, `ModeloJogo`).** Endurecimento do schema Prisma sem alterar comportamento das actions; **não toca em auth**. Migração `20260812094657_add_fk_constraints`.
  - **P1.4 — Índice `AtletaEscalao` validado por análise estática** (BD Supabase **inacessível** a partir do ambiente sandbox — `prisma db execute` devolve **`P1001`** ao tentar o `directUrl` `db.…supabase.co:5432`, pelo que `EXPLAIN (ANALYZE, BUFFERS)` **não pôde ser executado**; conclusão obtida por auditoria estática das *actions*, não por plano de execução real): auditadas as 20 utilizações de `AtletaEscalao`. As consultas de leitura por escalão — **plantel** (`lib/actions/atletas.ts`, a **dominante**: `{ escalaoId, epocaId, estado: "ATIVO", atleta: { ativo, clubeId } }`), contagens e `groupBy` de analíticos (`analise.ts`, `relatorios.ts`), números por atleta (`treinos.ts`, `jogos.ts`), validações de convocatória (`jogos.ts`) e guarda de integridade ao apagar escalão (`escaloes.ts`) — filtram **sempre por `escalaoId`** (igualdade ou `IN`), quase sempre acompanhado de `epocaId` e `estado="ATIVO"`. O índice composto **`@@index([escalaoId, epocaId, estado])`** é **óptimo** para este padrão: pela *prefix rule* do B-tree cobre todos os prefixos usados (`escalaoId` → `escalaoId+epocaId` → `escalaoId+epocaId+estado`) e coloca a coluna de menor cardinalidade (`estado`, 3 valores) por último. As restantes consultas **lideram por `atletaId`** (transferência/histórico/percurso em `participacoes.ts`; validações pontuais em `jogos.ts`) e são servidas pelo índice complementar **`@@index([atletaId, epocaId])`** e pelo `@@unique([atletaId, escalaoId, epocaId])`. **Nenhuma** consulta filtra por `epocaId` **isolado** nem por `clubeId` diretamente em `AtletaEscalao` (os filtros por clube fazem-se por *join* a `Atleta`/`Escalao`, servidos pelos índices dessas tabelas), pelo que **não** se justifica índice adicional em `[epocaId, estado]`. **Cenário A — índice já óptimo: sem alteração de `schema.prisma` e sem migração** para `AtletaEscalao`. Recomenda-se confirmar o plano com `EXPLAIN (ANALYZE, BUFFERS)` quando a BD estiver acessível (ambiente com dados reais).
  - **P1.5 — Constraints de FK adicionadas** (campos anteriormente *scalar-only*, com risco de dados órfãos): `EventoJogo.atletaId` → `Atleta` (**Cascade** — apagar o atleta apaga os seus eventos de jogo; novo `@@index([atletaId])`); `Planeamento.clubeId` → `Clube` (**Cascade**; novo `@@index([clubeId])`); `Competicao.clubeId` → `Clube` (**Cascade**); `ObservacaoAdversario.clubeId` → `Clube` (**Cascade**); `ModeloJogo.clubeProprietarioId` → `Clube` (**SetNull** — segue o padrão de `escalaoId`/`epocaId` do próprio modelo: se o clube for removido, a metodologia degrada para pessoal/portátil do autor, preservando o conteúdo). Todas as *actions* de escrita já definem estes campos com valores válidos do contexto autenticado (`perm.ctx.clube.id`, atleta validado por convocatória/participação), pelo que as constraints são rede de segurança sem alterar a UX; nenhum código de action precisou de mudança. (Nota: `EventoJogo.atletaSecundarioId` e `ObservacaoAdversario.escalaoId` permanecem *scalar-only* — fora do âmbito desta task.)
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run lint` **0 erros/0 avisos**; `npm run test` **678/678 a passar**. P1.4 não gera migração (índice já óptimo). A migração de P1.5 (`20260812094657_add_fk_constraints`) foi **criada** mas **não pôde ser aplicada** a partir do sandbox — a BD Supabase remota está inacessível (`P1001` no `directUrl`); aplicação diferida para ambiente com acesso via `prisma migrate deploy`.

- **2026-08-12** — **Legibilidade da landing pública e da impressão/PDF de relatórios (§12.0/§12.4, §10.6/§10.7).** Correções de contraste para visitantes anónimos e para a impressão, sem alterar estrutura, redesign, schemas, actions ou base de dados; **não toca em auth**. Alterações concentradas em `app/globals.css` (partilhado) + uma classe de scope em `app/page.tsx`.
  - **P1.1 — Impressão/PDF de relatórios partilhados (`/r/[token]`):** o tema base do produto é escuro e o bloco `.dark` remapeia o texto para tons claros; ao imprimir ou guardar como PDF, esse texto ficava ilegível sobre papel branco. Novo bloco **`@media print`** em `app/globals.css` que força tema claro de alto contraste **apenas na impressão** (fundo branco em `html/body` e superfícies `bg-white`/`bg-cinza-50/100/200`/`app-surface`; texto `text-cinza-*` e `h1–h3` em tons escuros; bordas/separadores em cinza claro; sombras removidas), usando `!important` para vencer os remaps de `.dark` independentemente do tema persistido. Garante ainda a impressão das cores de acento (`print-color-adjust: exact` em `svg` e nos *badges*/textos verde/âmbar/vermelho), preservando os gráficos SVG (cor do clube via `--cor-primaria`) e os *badges* de resultado. Sem qualquer efeito no ecrã.
  - **P1.2 — Landing pública (`app/page.tsx`) legível para anónimos:** a landing foi desenhada para tema claro, mas um visitante não autenticado herda o *default* escuro (`next-themes`, `defaultTheme="dark"`) e o remapeamento `.dark` das utilidades claras, deixando secções, preços e CTAs de baixo contraste. Adicionada a classe de scope **`landing-root`** ao `<main>` da landing e um bloco em `app/globals.css` que fixa os tons claros da marca **dentro da landing** (`.dark .landing-root .bg-white`/`.bg-cinza-50`/`.text-cinza-900/700/600/500`/`.border-cinza-200`), vencendo por especificidade (0,3,0 vs 0,2,0) os remaps globais. As secções intencionalmente escuras (hero e footer, com cores *inline*) mantêm-se inalteradas. Sem redesign nem alteração de estrutura.
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **todos passam** (sem novos testes — alterações puramente de CSS/apresentação).

- **2026-08-12** — **Integridade sessão↔periodização: só treinos NORMAL podem ter `planeamentoId` (§4/Grupo B, model `Sessao`).** A regra "só NORMAL liga à periodização" era apenas documental — nada impedia que uma sessão `ABERTO`/`CAPTACAO`/`EVENTO` gravasse um `planeamentoId`, corrompendo os dados de periodização. Passa a ser imposta no servidor por **dupla validação**:
  - **Schema (`lib/schemas/treino.ts`):** `sessaoSchema` ganha um `superRefine` que rejeita `planeamentoId` quando `tipoSessao !== "NORMAL"`, com erro no campo `planeamentoId` ("Só treinos normais podem estar associados a uma periodização.").
  - **Guarda nas actions (`lib/actions/treinos.ts`):** `criarSessao` e `atualizarSessao` reforçam a mesma regra após o parse (defesa em profundidade), devolvendo `erro(...)` com `camposInvalidos.planeamentoId` antes de qualquer escrita.
  - **Verificação:** `npm run typecheck` **0 erros**; `npm run test` **602/602** (2 novos testes em `tests/schemas.test.ts`: rejeita `planeamentoId` numa sessão `ABERTO`; aceita numa `NORMAL`).
...
- **2026-08-06** — **F13 — Polish transversal de experiência (§13.1, §12.0, §16 fase 23 — subconjunto).** Refinamento da UI existente, sem funcionalidade nova. Sem alterações a schemas, actions ou base de dados.
  - **Meta titles em todas as páginas:** `app/layout.tsx` passa a usar `title` com *template* `{ default: "Mister", template: "Mister – %s" }`; as **43 páginas** de `app/(app)/**` ganham `export const metadata: Metadata = { title: "…" }` (ex.: `Mister – Plantel`, `Mister – Detalhe do jogo`, `Mister – Definições · Épocas`). Títulos em pt-PT alinhados com a navegação.
...
- **2026-08-02** — **Rebranding: Mister → Mister + nova identidade visual.** Marca fixa + cor do clube dinâmica; Bricolage Grotesque + Inter; neutros quentes; §12 atualizada.
